...
# save exp.sub to perform PCA on whole data
write.table(exp.sub, file = "expression_alldata_551.txt", row.names = T, quote = F)
...
res.degs=res.df[res.df$padj< 0.01 & abs(res.df$log2FoldChange)>log2(1.2),]
...
write.table(exp.degs, file = "exp_res_degs_squamous[done].txt", quote = F, row.names = T)
...
par(mfrow=c(1,1))
with(res.df, plot(log2FoldChange, -log10(pvalue), pch=20, main="Volcano plot", xlim=c(-3,3)))

# Add colored points: blue if padj<0.01, red if log2FC>1 and padj<0.05)
#with(subset(res.df, padj<.01 ), points(log2FoldChange, -log10(pvalue), pch=20, col="blue"))
with(subset(res.df, padj< 0.01 & abs(log2FoldChange)>log2(1.2)), points(log2FoldChange, -log10(pvalue), pch=20, col="red"))
